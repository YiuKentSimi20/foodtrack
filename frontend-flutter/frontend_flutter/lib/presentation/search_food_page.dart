...
class _SearchFoodPageState extends State<SearchFoodPage> {
  ...
  Future<void> _onSelectAliment(AlimentDto aliment) async {
                          ...
                          MaterialPageRoute(
                            builder: (_) => AlimentDetailsPage(
                              ...
                              protein100g: (aliment.protein100g ?? 0)
                                  .toDouble(),
                              carbohydrates100g:
                                  (aliment.carbohydrates100g ?? 0).toDouble(),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: loading
                      ...
                      : () async {
                          final grams =
                              int.tryParse(gramsController.text.trim()) ?? 0;
                          if (grams <= 0) {
                            setStateSB(
                              () => localError = 'Introdu un gramaj valid (>0)',
                            );
                            return;
                          }
    ...
    if (added == true && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Alimentul a fost adaugat la masa')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Adauga un aliment')),
      body: Padding(
          ...
          children: [
            Card(
              child: ListTile(
                ...
                onTap: () async {
                  final createdAliment = await Navigator.of(context)
                      .push<AlimentDto>(
                        MaterialPageRoute(
                          builder: (_) => const CreateAlimentPage(),
                        ),
                      );
                  ...
                  if (createdAliment != null && mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Alimentul a fost creeat',
                        ), // textul pe care vrei să-l afișezi
                      ),
                    );
                ...
                onTap: () async {
                  ...
                  if (scannedCode != null && scannedCode.isNotEmpty) {
                    ...
                    try {
                      ...
                      setState(
                        () => _results = [aliment],
                      ); // înlocuiește lista cu rezultatul
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Aliment gasit pentru codul de bare: $scannedCode"',
                          ),
                        ),
                      );
                    } catch (e) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Nu s-a găsit aliment cu codul: $scannedCode',
                          ),
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, i) {
                  ...
                  return ListTile(
                    leading: Image(
                      image: AssetImage(
                        a.categorie?.iconPath ??
                            AlimentCategory.altele.iconPath,
                      ),
                      width: 25,
                      height: 25,
                    ),
                    title: Text(
                      a.productName ?? 'Aliment #${a.id}',
                      maxLines: 1,
                      ...
                      children: [
                        Column(
                          ...
                          children: [
                            if (a.brands != null)
                              Text(
                                a.brands!.substring(0, a.brands!.length > 25 ? 25 : a.brands!.length),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
                        ...
                        a.nutritionScore == null
                            ? const SizedBox.shrink()
                            : a.nutritionScore == NutritionScore.UNKNOWN
                            ? const SizedBox.shrink()
                            : Image(
                                image: AssetImage(a.nutritionScore!.iconPath),
                                width: 25,
                                height: 25,
                              ),
                      ],
